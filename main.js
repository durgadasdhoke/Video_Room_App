let APP_ID = '81c1ebbf06514d509328f80fe41fc1f4';
...
let urlParmas = new URLSearchParams(querryString);
...
let handleMessageFromPeer = async (message, uid)=>{
...
let createPeerConnection = async (uid)=>{
    ...
    peerConnection.onicecandidate = async (event)=>{
        if(event.candidate){
            client.sendMessageToPeer({text:JSON.stringify({'type':'candidate','candidate':event.candidate})}, uid);
...
let createOffer = async (uid)=>{
    ...
    client.sendMessageToPeer({text:JSON.stringify({'type':'offer','offer':offer})}, uid);
...
let createAnswer = async (uid, offer)=>{
    ...
    client.sendMessageToPeer({text:JSON.stringify({'type':'answer','answer':answer})}, uid);
}
